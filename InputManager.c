...
struct InputManager
{
    KeyState    m_key_state[KeyId_Max - KeyId_Min];
...
InputManager* InputManager_Create(const tchar* local_name)
{
    ...
    for(uint32 i=0, max_i = ARRAY_SIZE(input_manager->m_key_state); i<max_i; i++)
    {
        KeyState* key_state = &input_manager->m_key_state[i];
        key_state->m_key_id         = KeyId_Min + i;
